package ca.sheridancollege.pereir47.security;
...
// and create sample accounts to be used in our application
	...
	public JdbcUserDetailsManager jdbcUserDetailsManager() throws Exception {
		...
		User u2 = new User("Andrew", encodedPassword, authManager);
...
//configure access to the static resources
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/resources/**").and().ignoring().antMatchers("/h2-console/**").and()
		.ignoring().antMatchers("/css/**").and().ignoring().antMatchers("/js/**");
	}

	@Override
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").permitAll().and().authorizeRequests().antMatchers("/register")
				.permitAll().and().authorizeRequests().antMatchers("/home").hasAnyRole("USER", "MANAGER").and()
				.authorizeRequests().antMatchers("/admin").hasAnyRole("MANAGER").anyRequest().authenticated().and()
